...
class OrderController {
    ...
    async store(req, res) {
        ...
        try {
            const proServiceFound = await ValidateLib.hasOne(ProService, order.proService);
    ...
    async search(req, res) {
        ...
        if (!(await schema.isValid(params))) return responseHandler.badRequest(res);
    ...
    async delete(req, res) {
        ...
        try {
            ...
            if (!orderFound) return res.status(400).json( { message: 'Order not Found' });
    ...
    async update(req, res) {
        ...
        if(!(await schema.isValid(order))) return res.status(400).json({ message: 'Bad Request' });
        ...
        try {
            ...
            if (proService) {
                ...
                if(!proServiceFound) return res.status(400).json({ message: 'Pro Service not found'});
            }
